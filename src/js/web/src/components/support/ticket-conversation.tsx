...
export function TicketConversation({ ticketId, scrollToBottom = false, firstUnreadIndex }: TicketConversationProps) {
      ...
      <Card className="border-dashed">
        <CardContent className="flex flex-col items-center gap-3 py-12 text-center">
          ...
          <div>
            ...
            <p className="mt-1 text-sm text-muted-foreground/70">Be the first to reply to this ticket.</p>
          </div>
        </CardContent>
      </Card>
    )
    ...
    <div className="relative">
      ...
      <div className="mb-4 flex items-center justify-between">
        <div className="flex items-center gap-2 text-sm text-muted-foreground">
          ...
          <span>
            {hideSystem ? replyCount : totalCount} {(hideSystem ? replyCount : totalCount) === 1 ? "message" : "messages"}
          </span>
        ...
        {systemCount > 0 && (
          <Button variant={hideSystem ? "secondary" : "ghost"} size="sm" className="h-7 gap-1.5 text-xs" onClick={() => setHideSystem((prev) => !prev)}>
      ...
      <div ref={containerRef} className="relative max-h-[70vh] space-y-4 overflow-y-auto scroll-smooth">
        <div ref={topRef} />
        {visibleMessages.map((message, index) => {
          ...
          const showUnreadLine = firstUnreadIndex !== undefined && originalIndex === firstUnreadIndex && originalIndex > 0
            ...
            <div key={message.id}>
              ...
              {showDateSeparator && message.createdAt && (
                <div className="flex items-center gap-3 py-2">
                  ...
                  <span className="shrink-0 text-xs font-medium text-muted-foreground">{formatDateLabel(message.createdAt)}</span>
                  <div className="h-px flex-1 bg-border" />
                </div>
              )}
              ...
              {showUnreadLine && (
                <div className="flex items-center gap-3 py-1">
                  ...
                  <span className="shrink-0 text-xs font-semibold text-primary">New messages below</span>
                  <div className="h-px flex-1 bg-primary/50" />
                </div>
              )}
              ...
              <div
                className={cn(
                  ...
                  index >= visibleMessages.length - 3 && prevCountRef.current > 0 && "delay-75",
                )}
              >
                <TicketMessage message={message} ticketId={ticketId} isFirstMessage={originalIndex === 0 && !message.isSystemMessage} />
              </div>
            </div>
          )
        })}
      ...
      {isLongConversation && !isNearBottom && (
        <div className="pointer-events-none absolute bottom-4 left-0 right-0 flex justify-center">
          <Button size="sm" variant="secondary" className="pointer-events-auto h-8 gap-1.5 rounded-full shadow-md text-xs" onClick={scrollToEnd}>
      ...
      {isLongConversation && !isNearTop && (
        <div className="pointer-events-none absolute right-2 top-12 flex justify-end">
          <Button size="sm" variant="outline" className="pointer-events-auto h-7 w-7 rounded-full p-0 shadow-sm" onClick={scrollToTop}>
