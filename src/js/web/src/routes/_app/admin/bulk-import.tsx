...
import { AlertCircle, ArrowRight, Check, CheckCircle2, Columns3, Download, FileSpreadsheet, HardDrive, Hash, Loader2, MapPin, Phone, Upload, Users, X } from "lucide-react"
...
import { useImportDevices, useImportExtensions, usePreviewDeviceImport, usePreviewExtensionImport } from "@/lib/api/hooks/bulk-import"
...
const EXTENSION_CSV_TEMPLATE = "extension_number,display_name\n" + "1001,Front Desk\n" + "1002,Sales Team\n"

const USER_CSV_TEMPLATE = "email,name,role\n" + "jdoe@example.com,Jane Doe,member\n" + "asmith@example.com,Alex Smith,admin\n"

const LOCATION_CSV_TEMPLATE = "name,address,city,state,zip,country\n" + "HQ,123 Main St,New York,NY,10001,US\n" + "Branch,456 Oak Ave,Chicago,IL,60601,US\n"

const PHONE_NUMBER_CSV_TEMPLATE = "number,label,type,assigned_to\n" + "+15551234567,Main Line,local,\n" + "+18005551234,Toll Free,toll_free,\n"
      ...
      {STEPS.map((step, idx) => {
        ...
        return (
          <div key={step.key} className="flex items-center gap-2">
            {idx > 0 && <ArrowRight className={cn("h-3.5 w-3.5", isComplete ? "text-emerald-500" : "text-muted-foreground/30")} />}
    ...
    >
      ...
      {file ? (
        ...
        <>
          ...
          <p className="mt-1 text-xs text-muted-foreground">Accepts .csv files up to 5 MB</p>
        </>
      )}
    </div>
  )
...
function ValidationSummary({ preview }: ValidationSummaryProps) {
  const validPercent = preview.totalRows > 0 ? Math.round((preview.validRows / preview.totalRows) * 100) : 0
      ...
      <CardContent className="space-y-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          ...
          <div className="rounded-lg border border-emerald-200 bg-emerald-50 px-4 py-3 text-center dark:border-emerald-900 dark:bg-emerald-950">
            <p className="text-2xl font-semibold text-emerald-600 dark:text-emerald-400">{preview.validRows}</p>
          ...
          >
            <p className={cn("text-2xl font-semibold", preview.errorRows > 0 ? "text-destructive" : "text-emerald-600 dark:text-emerald-400")}>{preview.errorRows}</p>
            <p className={cn("text-xs", preview.errorRows > 0 ? "text-destructive" : "text-emerald-600 dark:text-emerald-400")}>Errors</p>
          </div>
        </div>
        ...
        <div className="space-y-1.5">
          ...
          <div className="h-2 w-full overflow-hidden rounded-full bg-muted">
            <div
              className={cn("h-full rounded-full transition-all duration-500", validPercent === 100 ? "bg-emerald-500" : validPercent >= 80 ? "bg-amber-500" : "bg-destructive")}
        ...
        {preview.errorRows > 0 && (
          <div className="rounded-md border border-destructive/20 bg-destructive/5 p-3">
            <p className="mb-2 text-xs font-medium text-destructive">Rows with errors will be skipped during import:</p>
            <ul className="space-y-1">
              {preview.rows
                .filter((row) => row.errors && row.errors.length > 0)
                .map((row) => (
                  <li key={row.rowNumber} className="text-xs text-destructive">
                    <span className="font-mono">Row {row.rowNumber}:</span> {row.errors?.join("; ")}
                  </li>
                ))}
            </ul>
          </div>
  ...
  return (
    <div className="space-y-2">
        ...
        <div className="flex items-center gap-2">
          {isPending ? (
            ...
            <>
              ...
              <span className="font-medium text-emerald-600 dark:text-emerald-400">Import complete</span>
            </>
          ) : null}
        </div>
      ...
      <div className="h-2.5 w-full overflow-hidden rounded-full bg-muted">
        <div className={cn("h-full rounded-full transition-all", isPending ? "bg-primary duration-200" : "bg-emerald-500 duration-300")} style={{ width: `${progress}%` }} />
      </div>
    </div>
  )
}
      ...
      <CardHeader className="pb-3">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-3 text-sm">
            ...
            <span className="text-emerald-600 dark:text-emerald-400">{preview.validRows} valid</span>
            {preview.errorRows > 0 && (
              <>
                <Separator orientation="vertical" className="h-4" />
                <span className="text-destructive">{preview.errorRows} with errors</span>
              </>
            )}
          </div>
        </div>
          ...
          <Table aria-label="Import preview">
            ...
            <TableBody>
              {preview.rows.map((row) => (
                <TableRow key={row.rowNumber} className={cn(row.errors && row.errors.length > 0 && "bg-destructive/5")}>
                  <TableCell className="font-mono text-xs text-muted-foreground">{row.rowNumber}</TableCell>
                  <TableCell>
                    <Badge variant={actionBadgeVariant[row.action] ?? "outline"}>{row.action}</Badge>
                  </TableCell>
                  {columns.map((col) => (
                    <TableCell key={col} className="text-sm">
                      {(row.data as Record<string, string>)[col] || <span className="text-muted-foreground/40">--</span>}
                    </TableCell>
                  ))}
                  <TableCell>
  ...
  return (
    <div className="space-y-4">
      <Alert variant={hasErrors ? "warning" : "success"}>
        ...
        <AlertTitle>{hasErrors ? "Import completed with warnings" : "Import completed successfully"}</AlertTitle>
        <AlertDescription>
...
function ImportSection({
  ...
  const currentStep: ImportStep = result ? "import" : preview ? "import" : clientParse ? "preview" : "upload"
      ...
      <Card>
        ...
        <CardContent>
          <div className="flex flex-col items-center justify-center rounded-lg border border-dashed border-border/60 py-12">
            ...
            <p className="mt-1 text-xs text-muted-foreground/60">Bulk import for {title.toLowerCase().replace("import ", "")} is not yet available.</p>
            <Button variant="outline" size="sm" className="mt-4 gap-1.5" onClick={() => downloadCsv(templateCsv, templateFilename)}>
    ...
    <Card>
      <CardHeader>
        <div className="flex items-center justify-between gap-4">
          ...
          <Button variant="outline" size="sm" className="gap-1.5" onClick={() => downloadCsv(templateCsv, templateFilename)}>
            <Download className="h-3.5 w-3.5" />
            Sample CSV
          </Button>
        </div>
      ...
      <CardContent className="space-y-6">
        ...
        {/* File drop zone */}
        <FileDropZone file={file} onFileSelect={handleFileSelect} disabled={importMutation.isPending} />

        {/* Client-side CSV preview (immediate, first 5 rows) */}
        {clientParse && clientParse.rows.length > 0 && <CsvPreviewTable parsed={clientParse} />}

        {/* Column mapping display */}
        {clientParse && clientParse.headers.length > 0 && <ColumnMappingCard detectedHeaders={clientParse.headers} expectedColumns={expectedColumns} />}
        ...
        <div className="flex items-center gap-3">
          ...
          {(clientParse || preview || result) && (
            <Button variant="ghost" size="sm" onClick={handleReset} disabled={importMutation.isPending}>
              Reset
            </Button>
          )}
        </div>
        ...
        {/* Import progress */}
        <ImportProgress isPending={importMutation.isPending} result={result} />
...
const DEVICE_COLUMNS = ["name", "mac_address", "model", "manufacturer", "device_type", "sip_username", "ip_address"]
