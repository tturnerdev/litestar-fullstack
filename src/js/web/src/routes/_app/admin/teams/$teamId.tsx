  ...
  return (
    <PageContainer className="flex-1 space-y-8">
              ...
              <div className="flex flex-col items-center gap-5 sm:flex-row sm:items-end">
                <div className="rounded-full bg-background p-1 shadow-md ring-2 ring-background">
                  <Avatar className="h-24 w-24 text-3xl">
                    <AvatarFallback className="bg-primary/10 text-primary text-3xl font-semibold">{getTeamInitials(editing ? editName || data.name : data.name)}</AvatarFallback>
                  </Avatar>
                </div>
      ...
      <PageSection delay={0.15}>
        <SectionHeading icon={Users} title="Members" description={`${memberCount} member${memberCount !== 1 ? "s" : ""} in this team.`} />
      ...
      <PageSection delay={0.2}>
        <SectionHeading icon={Mail} title="Invitations" description="Pending and accepted team invitations." />
      ...
      <PageSection delay={0.25}>
        <SectionHeading icon={Shield} title="Admin Actions" description="Toggle team properties and manage access." />
              ...
              <div className="flex items-center justify-between py-4 first:pt-0 last:pb-0">
                <div className="space-y-0.5">
                  ...
                  <p className="text-xs text-muted-foreground">
                    {data.isActive ? "Team is active and accessible to all members." : "Team is deactivated and members cannot access it."}
                  </p>
                </div>
                <div className="flex items-center gap-3">
                  ...
                  <Switch checked={data.isActive ?? false} onCheckedChange={() => updateTeam.mutate({ is_active: !data.isActive })} disabled={updateTeam.isPending} />
                </div>
              </div>
            </CardContent>
          </Card>
      ...
      {/* Dialogs */}
      <DeleteTeamDialog teamId={teamId} teamName={data.name} open={deleteOpen} onOpenChange={setDeleteOpen} onDeleted={() => navigate({ to: "/admin/teams" })} />
...
function MembersCard({ members }: MembersCardProps) {
  if (members.length === 0) {
    return (
      <Card>
        <CardContent className="py-8 text-center text-sm text-muted-foreground">This team has no members.</CardContent>
      </Card>
    )
  }
                  ...
                  <TableCell className="font-medium">
                    <div className="flex items-center gap-2">
                      <Avatar className="h-7 w-7 text-xs">
                        <AvatarFallback className="bg-primary/10 text-primary text-xs">{(member.user.name ?? member.user.email).slice(0, 2).toUpperCase()}</AvatarFallback>
                      </Avatar>
                      <Link to="/admin/users/$userId" params={{ userId: member.user.id }} className="hover:underline">
...
function InvitationsCard({ teamId }: { teamId: string }) {
  ...
  if (data.items.length === 0) {
    return (
      <Card>
        <CardContent className="py-8 text-center text-sm text-muted-foreground">No invitations for this team.</CardContent>
      </Card>
    )
  }
            ...
            <TableBody>
              {data.items.map((inv) => (
                <TableRow key={inv.id}>
                  ...
                  <TableCell>
                    <Badge variant={inv.isAccepted ? "default" : "secondary"}>{inv.isAccepted ? "Accepted" : "Pending"}</Badge>
                  </TableCell>
                  <TableCell>
                    <Tooltip>
                      <TooltipTrigger asChild>
                        <span className="cursor-default text-muted-foreground">{formatRelativeTimeShort(inv.createdAt)}</span>
                      </TooltipTrigger>
