      ...
      <TableCell className="text-right">
        ...
        <AlertDialog open={confirmDelete} onOpenChange={(open) => !open && setConfirmDelete(false)}>
          <AlertDialogContent>
            <AlertDialogHeader>
              <AlertDialogTitle className="flex items-center gap-2">
                <AlertTriangle className="h-5 w-5 text-destructive" /> Remove ring group member?
              </AlertDialogTitle>
              <AlertDialogDescription>
                This will remove {ext?.displayName || ext?.extensionNumber || member.externalNumber || "this member"} from the ring group. You can re-add them later.
              </AlertDialogDescription>
            </AlertDialogHeader>
            <AlertDialogFooter>
...
function RingGroupDetailPage() {
  ...
  const hasEditPermission = canEdit("CALL_ROUTING_RING_GROUPS")

  const { data, isLoading, isError, refetch, isRefetching, dataUpdatedAt } = useRingGroup(ringGroupId)
            ...
            <SectionErrorBoundary name="Group Members">
              <Card className="border-border/60 bg-card/80 shadow-md shadow-primary/10">
                ...
                <CardContent className="space-y-4">
                  ...
                  {hasEditPermission && <AddMemberRow groupId={ringGroupId} extensions={allExtensions} />}
                </CardContent>
              </Card>
            </SectionErrorBoundary>
