...
import { AlertCircle, AlertTriangle, ChevronRight, Globe, Headphones, Info, KeyRound, Loader2, Lock, Network, Phone, Plug, ShieldAlert, ShieldCheck } from "lucide-react"
...
import { type ConnectionUpdate, useConnection, useUpdateConnection } from "@/lib/api/hooks/connections"
...
function EditConnectionPage() {
  ...
  useEffect(() => {
    if (data && !initialized) {
      ...
      const matchedPreset = providerPresets.find((p) => p.value === normalizedProvider || p.label.toLowerCase() === normalizedProvider)
  ...
  const activePreset = selectedPreset && selectedPreset !== "_custom" ? providerPresets.find((p) => p.value === selectedPreset) : null
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const resolvedProvider = selectedPreset && selectedPreset !== "_custom" ? selectedPreset : provider.toLowerCase()
  ...
  const isValid = name.trim() !== "" && provider.trim() !== "" && !errors.host && !errors.port
          ...
          breadcrumbs={
            <Breadcrumb>
              <BreadcrumbList>
                ...
                <BreadcrumbItem>
                  <BreadcrumbLink asChild>
                    <Link to="/connections/$connectionId" params={{ connectionId }}>
                      {data.name}
                    </Link>
                  </BreadcrumbLink>
                </BreadcrumbItem>
              ...
              <CardContent>
                <form onSubmit={handleSubmit} className="space-y-6">
                  ...
                  <div className="grid gap-4 md:grid-cols-2">
                    <div className="space-y-2">
                      ...
                      <div className="flex items-center justify-between">
                        {errors.name ? <FieldError message={errors.name} /> : <FieldHint>A descriptive name to identify this connection.</FieldHint>}
                  ...
                  <div className="grid gap-4 md:grid-cols-3">
                    <div className="space-y-2 md:col-span-2">
                      ...
                      <Input
                        ...
                        placeholder={
                          activePreset
                            ? activePreset.hostPlaceholder
                            : connectionType === "network"
                              ? "e.g., 192.168.1.1 or unifi.local"
                              : "e.g., pbx.example.com or https://api.example.com"
                        }
                        value={host}
                    ...
                    <div className="space-y-2">
                      ...
                      <Input
                        ...
                        max={65535}
                      />
                      {errors.port ? <FieldError message={errors.port} /> : <FieldHint>1 - 65535. Leave blank to use the default.</FieldHint>}
                    </div>
                  </div>
                    ...
                    <div className="space-y-4 rounded-lg border border-border/60 bg-muted/20 p-4">
                      ...
                      <div className="flex items-start gap-2 rounded-md bg-muted/40 px-3 py-2">
                        ...
                        <p className="text-xs text-muted-foreground">
                          Credentials are encrypted at rest and never displayed after saving. Leave fields blank to keep existing credentials unchanged.
                        </p>
                      </div>
                      {activePreset && <p className="text-xs text-muted-foreground mt-1">{activePreset.credentialsHint}</p>}
                      <div className="grid gap-4 md:grid-cols-2">
                  ...
                  <div className="space-y-4 rounded-lg border border-border/60 bg-muted/20 p-4">
                    ...
                    <p className="text-xs text-muted-foreground">Override global gateway defaults for this connection. Leave empty to use the global defaults.</p>
                    <div className="grid gap-4 md:grid-cols-2">
                      <div className="space-y-2">
              ...
              <Card className="border-border/40">
                ...
                <CardContent className="space-y-3">
                  <p className="text-xs leading-relaxed text-muted-foreground">
                    All credentials are encrypted using AES-256 before being stored. They are never exposed in API responses or logs.
                  </p>
                  <p className="text-xs leading-relaxed text-muted-foreground">
        ...
        <AlertDialog open={pendingAuthType !== null} onOpenChange={(open) => !open && cancelAuthTypeChange()}>
          <AlertDialogContent>
            <AlertDialogHeader>
              ...
              <AlertDialogDescription>
                Switching the authentication type will discard all saved credentials for this connection. Any new credential values you have entered will also be cleared. This
                cannot be undone.
              </AlertDialogDescription>
            </AlertDialogHeader>
            <AlertDialogFooter>
      ...
      <AlertDialog open={blocker.status === "blocked"} onOpenChange={() => blocker.reset?.()}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>You have unsaved changes to this connection. Are you sure you want to leave? Your changes will be lost.</AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
