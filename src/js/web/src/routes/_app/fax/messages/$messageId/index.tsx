 ...
 */
function inferStatusTransitions(status: FaxStatus | undefined, direction: "inbound" | "outbound"): FaxStatus[] {
  if (!status) return []
...
const STATUS_META: Record<string, { label: string; icon: React.ComponentType<{ className?: string }>; dotColor: string }> = {
...
function buildFaxTimelineEvents(message: FaxMessage): FaxTimelineEvent[] {
  ...
  if (message.createdAt) {
    events.push({
      ...
      description: message.direction === "inbound" ? `From ${message.remoteName ?? message.remoteNumber}` : `To ${message.remoteName ?? message.remoteNumber}`,
      icon: Plus,
  ...
  const finalMs = message.receivedAt ? new Date(message.receivedAt).getTime() : message.updatedAt ? new Date(message.updatedAt).getTime() : createdMs
  ...
  if (message.receivedAt) {
    ...
    const meta = finalStatus ? STATUS_META[finalStatus] : undefined
    events.push({
      id: "received",
      timestamp: message.receivedAt,
      label: message.direction === "inbound" ? (meta?.label ?? "Received") : (meta?.label ?? "Sent"),
      description: message.pageCount ? `${message.pageCount} page${message.pageCount !== 1 ? "s" : ""} transmitted` : undefined,
  ...
  if (message.deliveredToEmails && message.deliveredToEmails.length > 0 && message.status === "delivered") {
  ...
  if (message.updatedAt && message.updatedAt !== message.createdAt && message.updatedAt !== message.receivedAt) {
    events.push({
      id: "updated",
...
function FaxMessageDetailPage() {
  ...
  const remoteDisplay = data.remoteName ? `${data.remoteName} (${data.remoteNumber})` : data.remoteNumber
    ...
    <PageContainer className="flex-1 space-y-8">
      ...
      {data.status === "failed" && data.errorMessage && (
        <Alert variant="destructive">
          ...
          <AlertDescription className="font-mono text-xs">{data.errorMessage}</AlertDescription>
        </Alert>
      )}
              ...
              <div className="grid gap-4 text-sm md:grid-cols-2 lg:grid-cols-3">
                <div>
                  ...
                  <div className="mt-1 flex items-center gap-2">
                    {data.direction === "inbound" ? <ArrowDownLeft className="h-4 w-4 text-blue-500" /> : <ArrowUpRight className="h-4 w-4 text-violet-500" />}
                    <DirectionBadge direction={data.direction} />
                  </div>
                </div>
                <div>
                ...
                <div>
                  <p className="text-muted-foreground">{data.direction === "inbound" ? "From Number" : "To Number"}</p>
                  ...
                  <div className="md:col-span-2 lg:col-span-3">
                    ...
                    <div className="mt-1.5 flex flex-wrap gap-2">
                      {data.deliveredToEmails.map((email) => (
                        <Badge key={email} variant="outline" className="gap-1.5 font-mono text-xs">
          ...
          <Card>
            ...
            <CardContent>
              <div className="grid gap-4 text-sm md:grid-cols-2 lg:grid-cols-4">
                ...
                <TimestampField label={data.direction === "inbound" ? "Received" : "Sent"} value={data.receivedAt} />
              ...
              {data.errorMessage && data.status !== "failed" && (
                <div className="mt-4">
                  ...
                  <div>
                    ...
                    <p className="mt-1 font-mono text-xs text-red-600 dark:text-red-400">{data.errorMessage}</p>
                  </div>
                </div>
              )}
            </CardContent>
        ...
        <SectionErrorBoundary name="Message History">
          <Card>
            ...
            <CardContent className="space-y-6">
              ...
              {/* Audit Trail */}
              <div>
                <p className="mb-3 text-xs font-medium uppercase tracking-wider text-muted-foreground">Audit Trail</p>
                <EntityActivityPanel targetType="fax_message" targetId={messageId} />
              </div>
            </CardContent>
          </Card>
        </SectionErrorBoundary>
            ...
            <CardContent>
              {pdfLoading ? (
                ...
                <div className="overflow-hidden rounded-lg border border-border/60">
                  <iframe src={pdfUrl} title="Fax document preview" className="h-[600px] w-full border-none" />
                </div>
              ) : (
            ...
            <CardContent>
              <div className="grid gap-4 text-sm md:grid-cols-2 lg:grid-cols-3">
                ...
                <div>
                  ...
                  <div className="flex items-center gap-1">
                    {data.faxNumberId ? (
                      <Link to="/fax/numbers/$faxNumberId" params={{ faxNumberId: data.faxNumberId }} className="font-mono text-xs text-primary hover:underline">
                        {data.faxNumberId}
                      </Link>
                    ) : (
                ...
                <TimestampField label={data.direction === "inbound" ? "Received At" : "Sent At"} value={data.receivedAt} />
              </div>
            </CardContent>
          </Card>
        </SectionErrorBoundary>
      ...
      <AlertDialog open={deleteOpen} onOpenChange={setDeleteOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This will permanently delete this fax message from <strong>{data.remoteNumber}</strong> and its associated document. This action cannot be undone.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel onClick={() => setDeleteOpen(false)} disabled={deleteMutation.isPending}>
              Cancel
            </AlertDialogCancel>
            <AlertDialogAction
