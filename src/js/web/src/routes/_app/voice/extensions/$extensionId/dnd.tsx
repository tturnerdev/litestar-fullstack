...
import { AlertCircle, ArrowLeft, BellOff, BellRing, Calendar, Clock, Loader2, ShieldCheck } from "lucide-react"
...
function DndPage() {
  ...
  const extensionName = extension ? `${extension.displayName} (Ext. ${extension.extensionNumber})` : `Extension ${extensionId.slice(0, 8)}`
    ...
    <PageContainer className="flex-1 space-y-8">
      ...
      <PageSection>
        <SectionErrorBoundary name="DND Status">
          <Card className={isEnabled ? "border-destructive/40 bg-destructive/5" : "border-emerald-500/30 bg-emerald-50/50 dark:bg-emerald-950/10"}>
            <CardContent className="py-8">
              <div className="flex flex-col items-center gap-6 text-center md:flex-row md:text-left">
                {/* Icon */}
                <div
                  className={`flex h-20 w-20 shrink-0 items-center justify-center rounded-full ${
                    isEnabled ? "bg-destructive/10 text-destructive" : "bg-emerald-100 text-emerald-600 dark:bg-emerald-900/30 dark:text-emerald-400"
                  }`}
                >
                  {isEnabled ? <BellOff className="h-10 w-10" /> : <BellRing className="h-10 w-10" />}
                </div>

                {/* Status info */}
                <div className="flex-1 space-y-2">
                  <div className="flex flex-col items-center gap-2 md:flex-row">
                    <h2 className="text-2xl font-semibold tracking-tight">{isEnabled ? "Do Not Disturb is Active" : "Do Not Disturb is Off"}</h2>
                    <Badge variant={isEnabled ? "destructive" : "secondary"} className="text-xs">
                ...
                <div className="flex flex-col items-center gap-2">
                  ...
                  <span className="text-xs text-muted-foreground">{toggleMutation.isPending ? "Updating..." : isEnabled ? "Click to disable" : "Click to enable"}</span>
                </div>
              </div>
            </CardContent>
          </Card>
            ...
            <Card>
              ...
              <CardContent>
                ...
                <p className="text-xs text-muted-foreground">
                  {mode === "scheduled" ? "Active during scheduled hours" : mode === "always" ? "Active around the clock" : "Not silencing any calls"}
                </p>
              </CardContent>
            </Card>
              ...
              <CardContent>
                {dnd.allowList && dnd.allowList.length > 0 ? (
                  <>
                    ...
                    <p className="text-xs text-muted-foreground">{dnd.allowList.length === 1 ? "1 number can bypass DND" : `${dnd.allowList.length} numbers can bypass DND`}</p>
                  </>
                ) : (
                  <>
