...
function ForwardingPage() {
  ...
  return (
    <PageContainer className="flex-1 space-y-8">
      ...
      {/* Add rule dialog */}
      <AddRuleDialog extensionId={extensionId} open={showAddDialog} onOpenChange={setShowAddDialog} />
            ...
            <TableRow key={rule.id}>
              ...
              <TableCell>
                <div className="flex flex-col gap-1">
                  <Badge variant={CONDITION_VARIANTS[rule.ruleType] ?? "outline"}>{RULE_TYPE_LABELS[rule.ruleType] ?? rule.ruleType}</Badge>
                  <span className="text-xs text-muted-foreground">{RULE_TYPE_DESCRIPTIONS[rule.ruleType] ?? ""}</span>
                </div>
              </TableCell>
              <TableCell>
                <Badge variant="outline">{DEST_TYPE_LABELS[rule.destinationType] ?? rule.destinationType}</Badge>
              </TableCell>
...
function AddRuleDialog({ extensionId, open, onOpenChange }: { extensionId: string; open: boolean; onOpenChange: (open: boolean) => void }) {
      ...
      <DialogContent className="sm:max-w-md">
        <form onSubmit={handleSubmit}>
          <DialogHeader>
            ...
            <DialogDescription>Create a new call forwarding rule. Choose when to forward and where calls should be sent.</DialogDescription>
          </DialogHeader>
...
function DeleteRuleDialog({ extensionId, rule, onOpenChange }: { extensionId: string; rule: ForwardingRule | null; onOpenChange: (open: boolean) => void }) {
        ...
        <AlertDialogHeader>
          ...
          <AlertDialogDescription>
            ...
            {rule && (
              <>
                {" "}
                This <span className="font-medium">{RULE_TYPE_LABELS[rule.ruleType]?.toLowerCase() ?? rule.ruleType}</span> rule forwards to{" "}
                <span className="font-mono text-foreground">{rule.destinationValue}</span>. This action cannot be undone.
              </>
            )}
          </AlertDialogDescription>
        </AlertDialogHeader>
