...
function NewPhoneNumberPage() {
  ...
  const numberFormatError = number.trim() !== "" && !/^\+\d+$/.test(number.trim()) ? "Must be E.164 format: + followed by digits (e.g., +15551234567)" : ""
  ...
  // Dirty check
  const formDirty = number.trim() !== "" || label.trim() !== "" || callerIdName.trim() !== "" || numberType !== "local" || !isActive
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <p className={cn("shrink-0 text-xs", number.length >= NUMBER_MAX ? "text-destructive" : "text-muted-foreground")}>
                  ...
                  <div className="space-y-2">
                    ...
                    <Input id="pn-label" placeholder="e.g., Main Line" value={label} onChange={(e) => setLabel(e.target.value)} maxLength={LABEL_MAX} />
                    <div className="flex items-center justify-between">
                      <p className="text-xs text-muted-foreground">Optional friendly name to identify this number.</p>
                      <p className={cn("shrink-0 text-xs", label.length >= LABEL_MAX ? "text-destructive" : "text-muted-foreground")}>
                  ...
                  <div className="space-y-2">
                    ...
                    <Input id="pn-caller-id" placeholder="e.g., Acme Corp" value={callerIdName} onChange={(e) => setCallerIdName(e.target.value)} maxLength={CALLER_ID_MAX} />
                    <div className="flex items-center justify-between">
                      <p className="text-xs text-muted-foreground">Name displayed to the recipient on outgoing calls.</p>
                      <p className={cn("shrink-0 text-xs", callerIdName.length >= CALLER_ID_MAX ? "text-destructive" : "text-muted-foreground")}>
                  ...
                  <div className="flex items-center gap-3 rounded-md border p-3">
                    <Switch id="pn-active" checked={isActive} onCheckedChange={setIsActive} />
                    <div className="space-y-0.5">
                      <Label htmlFor="pn-active" className="cursor-pointer">
                        Active
                      </Label>
                      <p className="text-xs text-muted-foreground">When disabled, the number will not receive or route calls.</p>
                    </div>
                  </div>
                  ...
                  {(number.trim() || callerIdName.trim()) && (
                    <div className="rounded-lg border bg-muted/50 p-4">
                      ...
                      <div className="flex flex-wrap items-center gap-x-3 gap-y-1 text-sm">
                        {number.trim() && <span className="font-mono">{number.trim()}</span>}
                        <span className="text-muted-foreground">{TYPE_OPTIONS.find((o) => o.value === numberType)?.label ?? numberType}</span>
                        {callerIdName.trim() && (
                          <>
